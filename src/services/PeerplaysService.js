...
class PeerplaysService {
  ...
  getBalance(accountName) {
    this.getFullAccount(accountName).then((account) => {
      ...
      if(account) {
        const balance = this.getBalanceByAsset(account.getIn(['balances']));
        this.store.dispatch(PeerplaysActions.setPeerplaysBalance(balance));
  ...
  formatBalance(unformatedBalance) {
    const balancePrecision = this.balancePrecision;

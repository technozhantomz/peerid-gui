...
import {Config, GenUtil, StorageUtil} from '../utility';
const ApiHandler = axios.create({withCredentials: true});
const apiRoot = Config.apiRoute;

// TODO: minify to get, post, patch, etc helper functions.
...
class AuthService {
  ...
  static peerplaysLogin(account) {
    let response;
    const query = `${apiRoot}api/v1/auth/peerplays`;
    ...
    return new Promise(async(resolve, reject) => {
      ...
      try {
        response = await ApiHandler.post(query, querystring.stringify(body), headers);
        const fullAccount = response.data.result;
        ...
        return reject(errorObj[Object.keys(errorObj)][0].message);
      }
    });
  }
}

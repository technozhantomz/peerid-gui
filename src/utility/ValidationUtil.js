...
const PrivateValidationUtils = {
  ...
  peerplaysUsername(username) {
    const length = username.length;
    // eslint-disable-next-line no-unused-vars
    const digitOrLetterHyphenRegex = /^[0-9a-z-]*$/;
    // eslint-disable-next-line no-unused-vars
    const lowercaseRegex = /^[a-z]*$/;

    // eslint-disable-next-line no-unused-vars
    let validLength=false, startWithEndWithLowercase=false, digitOrLetterOrHyphen=false;

    if (length >= 1 && length <=63) {
      validLength = true;
    }

    if(lowercaseRegex.test(username[0]) && lowercaseRegex.test(username[length-1])) {
      startWithEndWithLowercase = true;
    }

    if(digitOrLetterHyphenRegex.test(username)) {
      digitOrLetterOrHyphen = true;
    }

    const errorBoxUsernameValidation = [
      {errorString: 'between 1 and 63 characters', success: validLength},
      {errorString: 'Must start with lowercase and end with lowercase', success: startWithEndWithLowercase},
      {errorString: 'Can only contain digit, letter, or hyphen', success: digitOrLetterOrHyphen}
    ];

    console.log('error box: ', errorBoxUsernameValidation);
    return {
      errors: errorBoxUsernameValidation,
      success: errorBoxUsernameValidation.filter((err) => !err.success).length <= 0
    };
  ...
  peerplaysPassword(password) {
    ...
    const errorBoxPasswordValidation = [
      {errorString: translate('errors.password.requirement.peerplaysLength'), success: minLength}
    ];
...
const ValidationUtil = {
  ...
  peerplaysPassword(password) {
    return PrivateValidationUtils.peerplaysPassword(password);
  },

  peerplaysUsername(username) {
    return PrivateValidationUtils.peerplaysUsername(username);
  }
};
